  ...
  $(function(){
   let banner = {
      ...
      rollInit: function (newinterval) {
          ...
          //현재 배너
          let firstitem = document.querySelector('.rolling p');
          if(firstitem){
              firstitem.classList.add('currentroll');
          }
          //다음 배너
          let seconditem = document.querySelectorAll('.rolling p')[1];
          if(seconditem){
              seconditem.classList.add('nextroll');
          }
          //이전 배너
          document.querySelector('.rolling p:last-child').classList.add('prevroll');
